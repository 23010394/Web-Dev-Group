...
if (!empty($error_message)) {
    ...
    echo "<p>Go back <a href='student_signup.php'>Sign up</a></p>";
...
$email = $_POST['email'];
$query = "SELECT COUNT(*) FROM student_login WHERE email = ?";
...
if ($email_count > 0) {
    ...
    echo "<p>Please go back and use a different email to <a href='student_signup.php'>Sign up</a></p>";
    exit; // Stop script execution
}

// Proceed with the insert if email is unique
$sql = "INSERT INTO student_login (name, email, password_hash) VALUES (?, ?, ?)";
...
if ($stmt->execute()) {
    header("Location: studentSignup_success.php");
